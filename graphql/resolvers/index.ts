...
export const resolvers = {
  Query: {
    ...
    allRadixStakedTokens: async (_: any, __: any, { dataSources }: any) => {
      ...
      if (status === "error") return console.log(response.error)
      const { result } = data
      const val = result.map((res) => ({ metric: { instance: "radix", validator_address: res.metric.address }, bondedToken: res.value[1] }))
      return val;
    },
    ...
    oasisCommission: async (_: any, __: any, { dataSources }: any) => {
      ...
      if (status === "error") return console.log(response.error)
      const { result } = data
      const val = result.map((res) => ({ metric: { instance: "oasis", validator_address: res.metric.identity }, commissionRate: res.value[1] }))
      return val;
    },
    oasisTVL: async (_: any, __: any, { dataSources }: any) => {
      ...
      if (status === "error") return console.log(response.error)
      const { result } = data
      const val = result.map((res) => ({ metric: { instance: "oasis" }, TVL: res.value[1] }))
      return val;
    },
